...
function getElementTransform(el) {
  ...
  while (current && current.type === 'tag') {
    const tr = current.attribs && current.attribs.transform;
    if (tr) {
      const parsed = parseTransform(tr);
      if (result) {
        const { a, b, c, d, e, f } = parsed;
        const { a: na, b: nb, c: nc, d: nd, e: ne, f: nf } = result;
        result = {
          a: a * na + c * nb,
          b: b * na + d * nb,
          c: a * nc + c * nd,
...
function normalize(svgContent) {
  ...
  const minContentSize = canvasSize - 2 * padding;
  ...
  const dev = Math.abs(aspectRatio - 1);
  let scale, tx, ty, canvasW, canvasH;

  if (dev <= 0.05) {
    scale = minContentSize / Math.max(contentW, contentH);
    if (scale < 0.44 && Math.max(contentW, contentH) < 400) {
      const bigger = Math.min(minContentSize + 8, canvasSize - padding) / Math.max(contentW, contentH);
      scale = Math.min(bigger, 0.45);
      canvasW = Math.max(canvasSize, Math.ceil(contentW * scale + 2 * padding));
      canvasH = Math.max(canvasSize, Math.ceil(contentH * scale + 2 * padding));
    } else {
      canvasW = canvasSize;
      canvasH = canvasSize;
    }
    ...
    tx = (canvasW - scaledW) / 2;
    ty = (canvasH - scaledH) / 2;
  } else if (aspectRatio > 1) {
    scale = minContentSize / contentH;
    const scaledW = contentW * scale;
    canvasW = Math.ceil(scaledW + 2 * padding);
    canvasH = canvasSize;
    tx = (canvasW - scaledW) / 2;
    ty = padding;
  } else {
    scale = minContentSize / contentW;
    const scaledH = contentH * scale;
    canvasW = canvasSize;
    canvasH = Math.ceil(scaledH + 2 * padding);
    tx = padding;
    ty = (canvasH - scaledH) / 2;
  }

  const $result = load(`<svg xmlns="http://www.w3.org/2000/svg"${ns} viewBox="0 0 ${canvasW} ${canvasH}"></svg>`);
...
function cropContent(svgContent) {
  const $ = load(svgContent);
  const $svg = $('svg');
  const bounds = computeContentBounds($svg);

  const structural = new Set(['defs', 'style', 'metadata', 'title', 'desc', 'script', 'linearGradient', 'radialGradient', 'clipPath', 'mask', 'pattern', 'filter']);
  const children = $svg.children().toArray();
  const visual = children.filter(el => !structural.has(el.tagName));

  if (visual.length === 0) return svgContent;

  const needsCrop = (
    Math.abs(bounds.minX) > 0.01 ||
    Math.abs(bounds.minY) > 0.01 ||
    Math.abs(bounds.maxX - parseFloat($svg.attr('viewBox')?.split(' ')[2] || bounds.maxX)) > 0.01 ||
    Math.abs(bounds.maxY - parseFloat($svg.attr('viewBox')?.split(' ')[3] || bounds.maxY)) > 0.01
  );
  if (!needsCrop) return svgContent;

  for (const el of visual) {
    $(el).remove();
  }

  const $g = $('<g>').attr('transform', `translate(${-bounds.minX}, ${-bounds.minY})`);
  for (const el of visual) {
    $g.append(el);
  }
  $svg.append($g);
